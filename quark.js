...
------------------------------------------
青龙：
1.抓https://drive-m.quark.cn/1/clouddrive/capacity/growth/sign接口的kps、sign、vcode，用#连接，填写到quark_data，多账号用@分割

Surge、QuantumultX、Loon、Shadowrocket：
1.将获取ck脚本拉取到本地
2.打开网盘，手动点击一次签到，若提示获取ck成功则可以使用改脚本
3.关闭获取ck脚本，防止出现不必要的mitm。

ps：若以上获取ck操作失败，可以参照青龙的使用方法手动将参数填入boxjs
...
class UserInfo {
    constructor(str) {
        ...
        this.kps=this.ckInfo[0];
        this.sign=this.ckInfo[1];
        this.vcode=this.ckInfo[2];
        this.ckStatus = true;
        this.drawStatus = true;
        this.headers = {
            "Content-Type": "application/json",
            "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X; zh-cn) AppleWebKit/601.1.46 (KHTML, like Gecko) Mobile/21A329 Quark/6.6.3.1865 Mobile",
    ...
    async signin() {
        try {
            const options = {
                //签到任务调用签到接口
                url: `https://drive-m.quark.cn/1/clouddrive/capacity/growth/sign?kps=${this.kps}&sign=${this.sign}&vcode=${this.vcode}&uc_param_str=&dn=&fr=&pf=&bi=&ve=&ss=&bt=UC&bm=&ni=&ut=&pi=&nw=WIFI&mt=&sv=app&pc=&pr=ucpro&nn=&ch=&mi=&kp=`,
                //请求头, 所有接口通用
                headers: {
                    "Content-Type": "application/json",
                    "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X; zh-cn) AppleWebKit/601.1.46 (KHTML, like Gecko) Mobile/21A329 Quark/6.6.3.1865 Mobile",
...
async function getCookie() {
    if ($request && $request.method != 'OPTIONS') {
        ...
        let ckInfo=signUrl.split("?");
        let _params=ckInfo[1];
        let _obj=toJSON(_params);
        let kps=_obj.kps;
        let sign=_obj.sign;
        let vcode=_obj.vcode;
        if (kps&sign&vcode) {
            const tokenValue=kps+'#'+sign+'#'+vcode;
...
function toJSON(body) {
    ...
    for (let item of arr) {
        ...
        let _key = itemInfo[0];
        let _value = itemInfo[1];
        obj[`${_key}`] = _value
    }
    return obj;
}
//数据脱敏
function toAccess(str) {
    let obj = toJSON(str);
    obj.mt='';
    obj.pc ='';
    obj.nn = '';
